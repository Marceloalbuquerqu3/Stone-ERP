    ...
    <div className="flex h-screen bg-stone-50">
      ...
      <div className="flex-1 flex flex-col overflow-hidden">
        ...
        <main className="flex-1 overflow-y-auto p-6 lg:p-10">
          <Helmet><title>Preços - Stone ERP</title></Helmet>

          <div className="text-center max-w-2xl mx-auto mb-12">
            <h1 className="text-3xl font-bold text-stone-900 tracking-tight mb-4">Planos Flexíveis para sua Marmoraria</h1>
            <p className="text-stone-500 text-lg">Escolha o plano ideal para o tamanho do seu negócio. Cancele a qualquer momento.</p>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8 max-w-6xl mx-auto items-end">
            ...
            <Card className="rounded-2xl border-stone-200 shadow-sm bg-white hover:shadow-lg transition-all duration-300">
              <CardHeader className="p-8">
                ...
                <CardDescription className="mt-4 text-stone-500">
                  Para pequenas marmorarias que estão começando.
                </CardDescription>
              </CardHeader>
              <CardContent className="p-8 pt-0">
                <ul className="space-y-4">
                  {['Até 10 clientes', 'Até 10 orçamentos', 'Financeiro básico', 'Suporte por email'].map((feature) => (
                    <li key={feature} className="flex items-start">
                      <div className="flex-shrink-0">
                        <Check className="h-5 w-5 text-emerald-500" />
                      </div>
                      <p className="ml-3 text-sm text-stone-600">{feature}</p>
                    </li>
                  ))}
                </ul>
              </CardContent>
            ...
            <Card className="rounded-2xl border-stone-200 shadow-xl bg-white relative scale-105 z-10 border-t-4 border-t-stone-900">
              <div className="absolute top-0 right-0 -mt-3 mr-4">
                 <Badge className="bg-stone-900 text-white hover:bg-stone-800">MAIS POPULAR</Badge>
              </div>
              <CardHeader className="p-8">
                ...
                <CardDescription className="mt-4 text-stone-500">
                  Para marmorarias em crescimento que precisam de controle total.
                </CardDescription>
              </CardHeader>
              <CardContent className="p-8 pt-0">
                <ul className="space-y-4">
                  {['Clientes ilimitados', 'Orçamentos ilimitados', 'Financeiro completo', 'Gestão de Produção', 'Relatórios avançados'].map((feature) => (
                    <li key={feature} className="flex items-start">
                      <div className="flex-shrink-0">
                        <Check className="h-5 w-5 text-stone-900" />
                      </div>
                      <p className="ml-3 text-sm font-medium text-stone-900">{feature}</p>
                    </li>
                  ))}
                </ul>
              </CardContent>
            ...
            <Card className="rounded-2xl border-stone-200 shadow-sm bg-white hover:shadow-lg transition-all duration-300">
              <CardHeader className="p-8">
                ...
                <CardDescription className="mt-4 text-stone-500">
                  Para grandes empresas com múltiplas equipes.
                </CardDescription>
              </CardHeader>
              <CardContent className="p-8 pt-0">
                <ul className="space-y-4">
                  {['Tudo do plano PRO', 'Múltiplos usuários', 'Suporte prioritário', 'API de integração', 'Gestão de Estoque avançada'].map((feature) => (
                    <li key={feature} className="flex items-start">
                      <div className="flex-shrink-0">
                        <Check className="h-5 w-5 text-emerald-500" />
                      </div>
